...
| `load metadata for registry.redhat.io/...` fails with `x509: certificate signed by unknown authority` under **docker buildx** | The `mainframe-builder` builder runs BuildKit in its own isolated container (the `docker-container` driver) with its own trust store — separate from the host's docker CLI. A TLS-inspecting corporate proxy breaks the FROM-image pull even when `docker pull`/`podman pull` work fine on the host. Set `STUDIO_CA_CERT=/path/to/corp-root-ca.pem` before *Prepare Build Engine* (or before starting the Studio) — it injects the cert into the builder container and restarts it, and re-applies automatically every time the builder gets recreated. This is separate from `fetch-rpms.sh --ca-cert`, which only covers the RPM harvester's own throwaway container. |
