...
| `load metadata for registry.redhat.io/...` fails with `x509: certificate signed by unknown authority` under **docker buildx** | The `mainframe-builder` builder runs BuildKit in its own isolated container (the `docker-container` driver) with its own trust store — separate from the host's docker CLI. A TLS-inspecting corporate proxy breaks the FROM-image pull even when `docker pull`/`podman pull` work fine on the host. Set `STUDIO_CA_CERT=/path/to/corp-root-ca.pem` (use `sudo -E` so it survives sudo — pre-flight shows a *corporate CA* line when the server sees it), then click *Prepare Build Engine*: the builder is (re)created with a `buildkitd.toml` that trusts the CA for the Red Hat registries, BuildKit's official per-registry CA mechanism. A builder that exists but lacks the CA is detected and rebuilt automatically. This is separate from `fetch-rpms.sh --ca-cert`, which only covers the RPM harvester's own throwaway container. |
| `bootc-image-builder`: `could not access container storage` | bootc-image-builder only reads containers-storage (podman's store), never docker's. The generated script handles this automatically: under docker it runs `docker save \| podman load` and then runs the builder under podman — but **podman must be installed** alongside docker for that hand-off. |
